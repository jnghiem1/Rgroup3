...
# Canada has more orders than the United states. Approximately 5 times more orders overall and approximately 6 times more in sales revenue.
...
ordersByYear
# Shows the growth trend. Company is doing significantly better with every passing year indicating some level of progress and growth
```
...
# HomeOffice segment has the lowest number of orders and sales, however, mean HomeOffice has the highest unit sale value
# Consumer segment orders is about 4 times that of Home Office in Canada
```
...
# Consumer segment orders is only about 2 times that of Home Office in the United States
```
```{r} 
...
CustomerShipModes <- subset(completeDatadf, Segment=='Consumer')
CustomerShipModes

CustomerShipModesGroup <- CustomerShipModes %>% group_by( `Country`,`Ship Order` )  %>% 
  summarise( order_total = sum(Price))

CustomerShipModesGroup
```

```{r}
CorporateShipModes <- subset(completeDatadf, Segment=='Corporate')
CorporateShipModes

CorporateShipModesGroup <- CorporateShipModes %>% group_by( `Country`,`Ship Order` )  %>% 
  summarise( order_total = sum(Price))
CorporateShipModesGroup

```


```{r}
HomeOfficeShipModes <- subset(completeDatadf, Segment=='HomeOffice')
HomeOfficeShipModes

HomeOfficeShipModesGroup <- HomeOfficeShipModes %>% group_by( `Country`,`Ship Order` )  %>% 
  summarise( order_total = sum(Price))
HomeOfficeShipModesGroup

```

```{r} 

# Ship Order by Sales Segment

allOrders <- subset(completeDatadf)
allOrders

allOrdersShipModesGroup <- allOrders %>% group_by( `Segment`,`Ship Order` )  %>% 
  summarise( order_total = sum(Price))
allOrdersShipModesGroup


# The highest orders value came from Consumer/Standard Class shipping @ 116922.135	
# The lowest order value came from HomeOffice/SameDayClass @ 2977.456	
```
