...
```
```{r - part 5-2 AC} 
## segment and ship order breakdown
Seg<- group_project_data_nototal_final$Segment
SHO<- group_project_data_nototal_final$`Ship Order`
FTSS<- table(Seg, SHO)
barplot(FTSS, main = "Segment and Ship Order", col = c('blue', 'yellow','green'), legend = row.names(FTSS), args.legend = list(x="topleft"), xlab = 'Ship Order', ylab = 'Count', ylim = c(0,300),  beside = TRUE)
...
## Lineary regression model 2
## Develop a linear regression model. Use 𝛼 = 0.05. 

Country <- finaltb$Country_Segment_ShipOrder
Mean_Price <- finaltb$mean_Price
Segment <- finaltb$Country_Segment_ShipOrder
